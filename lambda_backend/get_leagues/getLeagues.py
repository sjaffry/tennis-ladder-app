...
# Custom encoder to handle datetime objects and format end_date
class DateTimeEncoder(json.JSONEncoder):
    def default(self, obj):
        if isinstance(obj, datetime):
            return obj.strftime('%b-%d-%Y')  # Format as Mon-DD-YYYY
...
def lambda_handler(event, context):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            sql_query = """
                    SELECT league_id, league_name, category, league_type, end_date
                    FROM league
